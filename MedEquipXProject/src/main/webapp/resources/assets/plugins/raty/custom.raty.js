(function ($) {
  ...
  if ($(".rating-score").length) {
    $(".rating-score").raty({ score: 3, scoreName: "entity[1,2,3,4,5]" });
  }
  if ($(".rating-score-name").length) {
    $(".rating-score-name").raty({ scoreName: "Rating" });
  }
  if ($(".rating-number").length) {
    $(".rating-number").raty({ number: 10 });
  }
  if ($(".rating-number-max").length) {
    $(".rating-number-max").raty({ numberMax: 5, number: 10 });
  }
  if ($(".rating-read-only").length) {
    $(".rating-read-only").raty({ readOnly: true });
  }
  if ($(".rating-read-only2").length) {
    $(".rating-read-only2").raty({ readOnly: true, score: 3 });
  }
  ...
  if ($(".rating-half-enable").length) {
    $(".rating-half-enable").raty({ score: 3.3 });
  }
  if ($(".rating-half-disable").length) {
    $(".rating-half-disable").raty({ halfShow: false, score: 3.3 });
  }
  if ($(".rating-half").length) {
    $(".rating-half").raty({ half: true });
  }
  if ($(".rating-hint").length) {
    $(".rating-hint").raty({ hints: ["a", null, "", undefined, "*_*"] });
  }
  if ($(".rating-path").length) {
    $(".rating-path").raty({ path: "assets/images/rating/" });
  }
  if ($(".rating-cancel").length) {
    $(".rating-cancel").raty({ cancel: true });
  }
